...
import { useSubscriptionQuery, useTripHistoryQuery, useAvailableRoutesQuery } from '@/hooks';
...
export default function HomeScreen() {
  ...
  const { data: subscription, refetch: refetchSubscription } = useSubscriptionQuery();
  const { data: tripHistory = [], refetch: refetchTripHistory } = useTripHistoryQuery();
  const { data: availableRoutes = [], refetch: refetchRoutes } = useAvailableRoutesQuery(
  ...
  const isLoading =
    user?.role === 'student' && !subscription && !tripHistory.length && !availableRoutes.length;

  const refreshing = false;

  const onRefresh = React.useCallback(async () => {
    await Promise.all([refetchSubscription(), refetchTripHistory(), refetchRoutes()]);
  }, [refetchSubscription, refetchTripHistory, refetchRoutes]);
  ...
  return (
    ...
    >
      ...
      >
          ...
          {availableRoutes.length > 0 ? (
            availableRoutes.slice(0, 5).map((route) => (
              <RouteCard
                ...
                isBooked={
                  subscription?.status === 'active' && subscription.driver_id === route.driverId
                }
              />
            ))
          ) : (
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>إحصائيات سريعة</Text>
          <View style={styles.quickStatsGrid}>
            <StatCard label="الرحلات القادمة" value={upcomingTrips} icon="truck" compact />
            <StatCard
