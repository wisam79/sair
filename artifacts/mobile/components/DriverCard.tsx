...
export function DriverCard({ driver, onSubscribe }: DriverCardProps) {
  ...
  const stars = [1, 2, 3, 4, 5].map((i) => (
    <Text
      key={i}
      style={{ color: i <= Math.round(rating) ? '#FFD700' : colors.border, fontSize: 12 }}
    >
      {i <= Math.round(rating) ? '★' : '☆'}
    </Text>
  ));
      ...
      <TouchableOpacity onPress={toggleExpand} activeOpacity={0.7} style={styles.header}>
        ...
        <View style={styles.headerText}>
          <Text style={[styles.name, { color: colors.foreground }]}>
            {driver.profile?.full_name ?? 'سائق جديد'}
          </Text>
          ...
          <View style={styles.ratingRow}>
            {stars}
            <Text style={[styles.ratingValue, { color: colors.mutedForeground }]}>
              ({rating.toFixed(1)})
            </Text>
          </View>
        ...
        <View style={styles.headerRight}>
          <View style={styles.statusBadge}>
            <Animated.View
              style={[
                styles.onlineDot,
                {
                  backgroundColor: driver.is_available ? colors.success : colors.mutedForeground,
                  opacity: pulseAnim,
                },
              ]}
            />
            <Text
              style={[
                styles.statusText,
                { color: driver.is_available ? colors.success : colors.mutedForeground },
              ]}
            >
          ...
          {driver.is_available && (
            <Text style={[styles.waitTime, { color: colors.mutedForeground }]}>~5 دق</Text>
          )}
          <FeatherIcon
        ...
        <View style={styles.expandedContent}>
          <View style={styles.detailsHeader}>
            <View style={styles.universityRow}>
              ...
              <Text style={[styles.universityText, { color: colors.foreground }]}>—</Text>
            </View>
          ...
          <Text style={[styles.plansTitle, { color: colors.mutedForeground }]}>
            خطط الاشتراك الشهري
          </Text>
          ...
          <View style={[styles.comparisonTable, { borderColor: colors.border }]}>
            <View style={[styles.comparisonHeader, { backgroundColor: colors.secondary }]}>
              <Text
                style={[styles.comparisonHeaderText, { flex: 1.2, color: colors.mutedForeground }]}
              >
                الميزة
              </Text>
              <Text style={[styles.comparisonHeaderText, { color: colors.mutedForeground }]}>
                أساسي
              </Text>
              <Text style={[styles.comparisonHeaderText, { color: colors.mutedForeground }]}>
                قياسي
            ...
            <View style={styles.comparisonRow}>
              <Text style={[styles.comparisonLabel, { flex: 1.2, color: colors.foreground }]}>
                السعر
              </Text>
              <Text style={[styles.comparisonValue, { color: colors.accent }]}>
                {(fare / 1000).toFixed(0)}k
              </Text>
              <Text style={[styles.comparisonValue, { color: colors.accent }]}>
                {(fare / 1000).toFixed(0)}k
              </Text>
                ...
                >
                  {isPremium ? (
                    ...
                    <View style={styles.planBtnContent}>
                      ...
                      <Text style={[styles.planName, { color: colors.foreground }]}>{p.label}</Text>
