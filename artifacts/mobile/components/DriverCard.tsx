...
export function DriverCard({ driver, onSubscribe }: DriverCardProps) {
  ...
  const plans: { plan: SubscriptionPlan; label: string; fare: number }[] = [
    { plan: "basic", label: "أساسي", fare: driver.basicFare },
    { plan: "standard", label: "قياسي", fare: driver.standardFare },
    { plan: "premium", label: "مميز", fare: driver.premiumFare },
  ];
    ...
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={styles.header}>
        ...
        <View style={styles.info}>
          ...
          <Text style={[styles.plate, { color: colors.mutedForeground }]}>{driver.vehiclePlate ?? "—"}</Text>
        </View>
      ...
      <View style={[styles.statsRow, { borderColor: colors.border }]}>
        <View style={styles.stat}>
          ...
          <Text style={[styles.statValue, { color: colors.foreground }]}>{Number(driver.rating).toFixed(1)}</Text>
        ...
        <View style={styles.stat}>
          ...
          <Text style={[styles.statValue, { color: colors.foreground }]} numberOfLines={1}>
            {(driver.university ?? "—").replace("جامعة ", "")}
          </Text>
...
const styles = StyleSheet.create({
  card: { borderRadius: 16, borderWidth: 1, padding: 16, marginBottom: 12, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  vehicle: { fontSize: 13, fontFamily: "Inter_400Regular", marginBottom: 1 },
  plate: { fontSize: 12, fontFamily: "Inter_400Regular" },
  ...
  planBtn: { flex: 1, borderRadius: 10, borderWidth: 1, paddingVertical: 10, alignItems: "center" },
  planName: { fontSize: 13, fontFamily: "Inter_600SemiBold", marginBottom: 2 },
  planFare: { fontSize: 15, fontFamily: "Inter_700Bold" },
