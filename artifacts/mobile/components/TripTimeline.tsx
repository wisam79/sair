...
import { format, formatArabicDate, formatTimeArabic, formatArabicDayName, arSA } from '@/lib/dates';
...
export default function TripTimeline({ trip }: TripTimelineProps) {
  ...
  const startTime = formatTimeArabic(trip.started_at ?? trip.trip_date);
  const startDate = (() => {
    const d = new Date(trip.trip_date);
    return `${formatArabicDayName(d)}، ${format(d, 'd MMMM', { locale: arSA })}`;
  })();

  const endTime = trip.ended_at ? formatTimeArabic(trip.ended_at) : null;
    ...
    <View style={[styles.container, { backgroundColor: colors.card }]}>
        ...
        <View style={[styles.timelineItem, { marginBottom: 0 }]}>
          ...
          <View style={styles.rightColumn}>
            <View style={styles.entryHeader}>
              ...
              {endTime && (
                <Text style={[styles.entryTime, { color: colors.muted }]}>{endTime}</Text>
              )}
            </View>
            <Text style={[styles.entryAddress, { color: colors.muted }]} numberOfLines={2}>
              الوجهة
      ...
      {isCompleted && (
        <View style={[styles.fareRow, { borderTopColor: colors.border }]}>
          ...
          <Text style={[styles.fareAmount, { color: colors.success }]}>{formatFare('0')}</Text>
        </View>
      )}
    </View>
  );
