...
import { createMoney, toNumberValue } from '@/lib/money';
...
export default function PriceBreakdown({ fare, label }: PriceBreakdownProps) {
  ...
  const { commission_amount_iqd } = useAppSettings();

  const commissionRate = Math.round((commission_amount_iqd / fare) * 100);
  const appCommission = commission_amount_iqd;
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>{label || 'تفاصيل السعر'}</Text>
      </View>

      <View style={styles.totalRow}>
        <Text style={[styles.totalAmount, { color: colors.text }]}>{formatCurrency(fare)}</Text>
      </View>
