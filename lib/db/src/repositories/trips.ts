...
export const tripRepository = {
  ...
  async findById(id: string): Promise<Trip | undefined> {
    const [result] = await db.select().from(tripsTable).where(eq(tripsTable.id, id));
    return result;
  },
  ...
  async update(id: string, data: Partial<typeof tripsTable.$inferInsert>): Promise<Trip> {
    const [result] = await db.update(tripsTable).set(data).where(eq(tripsTable.id, id)).returning();
    return result;
  },
  ...
  async getStudentStatus(tripId: string, studentId: string): Promise<TripStudent | undefined> {
    const [result] = await db
      .select()
      .from(tripStudentsTable)
      .where(and(eq(tripStudentsTable.tripId, tripId), eq(tripStudentsTable.studentId, studentId)));
    return result;
  },
  ...
  async updateStudentStatus(
    ...
    await db
      .update(tripStudentsTable)
      .set({ status })
      .where(and(eq(tripStudentsTable.tripId, tripId), eq(tripStudentsTable.studentId, studentId)));
  },

  async count(): Promise<number> {
    const [result] = await db.select({ count: sql<number>`count(*)` }).from(tripsTable);
