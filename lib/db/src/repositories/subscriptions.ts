...
export const subscriptionRepository = {
  ...
  async findByStudent(studentId: string): Promise<SubscriptionWithRelations[]> {
    const results = await db
      ...
      .leftJoin(driversTable, eq(subscriptionsTable.driverId, driversTable.id))
      .where(
        and(eq(subscriptionsTable.studentId, studentId), eq(subscriptionsTable.isDeleted, false)),
      )
      .orderBy(desc(subscriptionsTable.createdAt));
  ...
  async findByDriver(driverId: string): Promise<SubscriptionWithRelations[]> {
    const results = await db
      ...
      .leftJoin(profilesTable, eq(subscriptionsTable.studentId, profilesTable.id))
      .where(
        and(eq(subscriptionsTable.driverId, driverId), eq(subscriptionsTable.isDeleted, false)),
      )
      .orderBy(desc(subscriptionsTable.createdAt));
  ...
  async findByStatus(status: Subscription['status']): Promise<Subscription[]> {
    return db
      .select()
      .from(subscriptionsTable)
      .where(and(eq(subscriptionsTable.status, status), eq(subscriptionsTable.isDeleted, false)))
      .orderBy(desc(subscriptionsTable.createdAt));
  },
  ...
  async findAllFiltered({
    ...
    if (paymentStatus && paymentStatus !== 'all') {
      conditions.push(
        eq(subscriptionsTable.paymentStatus, paymentStatus as Subscription['paymentStatus']),
      );
    }
