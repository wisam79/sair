...
export const tripsTable = pgTable(
  'trips',
  {
    ...
    status: text('status', {
      enum: ['scheduled', 'driver_waiting', 'in_transit', 'completed', 'absent', 'cancelled'],
    })
      .notNull()
      .default('scheduled'),
...
export const tripStudentsTable = pgTable(
  'trip_students',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tripId: uuid('trip_id')
    ...
    status: text('status', { enum: ['waiting', 'picked_up', 'dropped_off', 'absent'] })
      .notNull()
      .default('waiting'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
