...
const describeDB = describe.skip; // Requires Supabase local or dedicated test DB with schema matching test expectations
...
describeDB('1. Logic Unit Tests', () => {
  it('يجب حساب تاريخ انتهاء الاشتراك بشكل صحيح (30 يوم)', async () => {
    // إنشاء طالب وسائق
    const student: InsertProfile = {
      fullName: 'أحمد محمد',
      phone: '+9647700000001',
      role: 'student',
    };

    const driver: InsertProfile = {
      fullName: 'سعيد علي',
      phone: '+9647700000002',
      role: 'driver',
    };

    const [createdStudent] = await db.insert(profilesTable).values(student).returning();
    const [createdDriver] = await db.insert(profilesTable).values(driver).returning();
    ...
    expect(createdSubscription.endDate.getTime()).toBeGreaterThanOrEqual(
...
describeDB('2. Constraint Validation Tests', () => {
  it('يجب رفض إنشاء مستخدم بدون اسم', async () => {
    ...
    await expect(db.insert(profilesTable).values(invalidUser)).rejects.toThrow();
  });

  it('يجب رفض إنشاء مستخدم برقم هاتف مكرر', async () => {
    const user1: InsertProfile = {
      fullName: 'علي حسين',
      phone: '+9647700000006',
      role: 'student',
    };

    await db.insert(profilesTable).values(user1);

    const user2: InsertProfile = {
      fullName: 'حسن علي',
      phone: '+9647700000006', // نفس رقم الهاتف
      role: 'driver',
    };

    await expect(db.insert(profilesTable).values(user2)).rejects.toThrow();
  });
...
describeDB('3. RLS Security Tests', () => {
  ...
  it('يجب أن يرى الطالب اشتراكاته فقط', async () => {
    ...
    const student1Subscriptions = await db
      .select()
  ...
  it('يجب منع تعديل اشتراك طالب آخر', async () => {
    ...
    const attemptedUpdate = await db
      .select()
      .from(subscriptionsTable)
      .where(
        and(
...
describeDB('4. Concurrency & Race Condition Tests', () => {
  it('يجب منع الحجز الزائد عند طلبات متزامنة', async () => {
    ...
    const students = await Promise.all([
      db
        .insert(profilesTable)
        .values({ fullName: 'طالب 1', phone: '+9647700000016', role: 'student', gender: 'male' })
        .returning(),
      db
        .insert(profilesTable)
        .values({ fullName: 'طالب 2', phone: '+9647700000017', role: 'student', gender: 'female' })
        .returning(),
      db
        .insert(profilesTable)
        .values({ fullName: 'طالب 3', phone: '+9647700000018', role: 'student', gender: 'male' })
        .returning(),
    ]);

    // محاكاة حجز متزامن باستخدام Transaction
    const bookingResults = await Promise.allSettled(
      students.map(async (studentArr, index) => {
